...
def get_smart_leverage(symbol: str, atr_pct: float, score: float) -> int:
    ...
    if base in ["BTC","ETH"]:          base_lev = 10
    elif base in ["BNB","SOL"]:        base_lev = 8
    elif atr_pct < 2.0:                base_lev = 8
    elif atr_pct < 4.0:                base_lev = 5
    else:                              base_lev = 4
    if score >= 99:      multiplier = 2.0
    elif score >= 98:    multiplier = 1.75
    elif score >= 97:    multiplier = 1.5
    elif score >= 96:    multiplier = 1.25
    else:                multiplier = 1.0
    return min(int(base_lev * multiplier), 20)

def get_signal_grade(score, whale, oi_rising, tf_score, volume_ok, rsi_ok, funding_ok) -> str:
    points = 0
...
def get_timeframe_score(symbol, direction):
    ...
    htf_4h=get_htf_trend(symbol,"4h"); htf_1h=get_htf_trend(symbol,"1h")
    if htf_4h != 0 and htf_4h != di: return -1
    score = 0
    if htf_4h == di: score += 2
    if htf_1h == di: score += 1
    return score

def get_structure_sl(klines, direction, entry, atr):
    lows=[float(k[3]) for k in klines[-20:]]; highs=[float(k[2]) for k in klines[-20:]]
    if direction=="BUY": return min(min(lows)*0.998, entry-atr*ATR_SL_MULTIPLIER)
    return max(max(highs)*1.002, entry+atr*ATR_SL_MULTIPLIER)

def check_circuit_breaker():
    global daily_losses, last_reset_day
    today = datetime.now(IST).date()
    if today != last_reset_day: daily_losses=0; last_reset_day=today
...
def increment_daily_losses():
    ...
    if daily_losses >= MAX_DAILY_LOSSES:
        send_telegram(f"🚨 <b>{BOT_HEADER}</b>\nCIRCUIT BREAKER ACTIVE\n3 losses today. No more signals until tomorrow. 🛡️")

def learn_from_trade(coin, pattern, result, pnl, market_condition, tf_score):
    global learning_notes, market_memory, consecutive_loss_patterns
    ...
    for entry in trade_journal:
        if entry.get("market_condition")==market_condition and entry.get("result")=="WIN":
            p=entry.get("pattern","Unknown"); wins_by_pattern[p]=wins_by_pattern.get(p,0)+1
    if wins_by_pattern:
    ...
    if result=="LOSS":
        ...
        if cl>=CONSEC_LOSS_SUSPEND:
            ...
            send_telegram(f"🧠 <b>{BOT_HEADER} Pattern Suspended</b>\n\n'{pattern}' has {cl} consecutive losses.\nSuspended 24 hours. 🔒")
    else:
        consecutive_loss_patterns[pattern]["consecutive_losses"]=0
        consecutive_loss_patterns[pattern]["suspended_until"]=None

    stats=pattern_stats.get(pattern,{}); signals=stats.get("signals",0); note=None
    if signals>=5:
        win_rate=(stats["wins"]/signals)*100
        if result=="LOSS" and win_rate<45:
            note=f"⚠️ '{pattern}' only {win_rate:.1f}% WR after {signals} signals in {market_condition}. Consider avoiding."
        elif result=="WIN" and win_rate>70:
            note=f"✅ '{pattern}' strong — {win_rate:.1f}% WR in {market_condition}. Keep prioritising."
        elif result=="LOSS" and market_condition=="sideways":
            note=f"📊 Loss on '{pattern}' in sideways. Struggles when BTC ranges. Reduce size."
        elif tf_score<3 and result=="LOSS":
            note=f"📉 Loss on {coin} with TF score {tf_score}/3. Higher loss rate when not fully aligned."
...
def generate_weekly_insight():
    ...
    if not week_trades: return "Not enough data for weekly insight."
    wins=[t for t in week_trades if t["result"]=="WIN"]
    losses=[t for t in week_trades if t["result"]=="LOSS"]
    total=len(week_trades); wr=(len(wins)/total*100) if total>0 else 0
    day_wins={}
    for t in wins:
        d=t["date"]; day_wins[d]=day_wins.get(d,0)+1
    best_day=max(day_wins,key=day_wins.get) if day_wins else None
    win_pats=[t["pattern"] for t in wins]; loss_pats=[t["pattern"] for t in losses]
    ...
    if sideways_losses>=2: msg+=f"📊 {sideways_losses} losses in sideways — reduce activity when BTC ranges\n"
    if wr>=70:     msg+="\n🔥 Excellent week!"
    elif wr>=50:   msg+="\n✅ Decent week. Keep following signals."
    else:          msg+="\n⚠️ Tough week. Review learning notes and be patient."
    return msg

def get_learning_text():
    if not learning_notes: return f"🧠 <b>{BOT_HEADER} Learning</b>\n\nNo insights yet."
    ...
    suspended=[p for p,d in consecutive_loss_patterns.items()
               if d.get("consecutive_losses",0)>=CONSEC_LOSS_SUSPEND and d.get("suspended_until")]
    text+=f"\n<b>Suspended:</b> {', '.join(suspended) if suspended else 'None'}\n"
    text+=f"\n<b>Latest Insights:</b>\n\n"
...
def get_crypto_news():
    ...
    if NEWS_API_KEY:
        try:
            res=requests.get("https://cryptopanic.com/api/v1/posts/",
                             params={"auth_token":NEWS_API_KEY,"kind":"news","filter":"hot"},timeout=10)
            if res.status_code==200:
    ...
    try:
        ...
        if res.status_code==200:
            data=res.json()["data"]; latest=data[0]; yesterday=data[1] if len(data)>1 else None
            fng_val=int(latest["value"]); fng_cls=latest["value_classification"]; change=""
            if yesterday:
                diff=fng_val-int(yesterday["value"]); change=f" ({'+' if diff>=0 else ''}{diff} vs yesterday)"
    ...
    try:
        ...
        if btc: items.append(f"₿  BTC: ${btc:,.2f}")
        if eth: items.append(f"Ξ  ETH: ${eth:,.2f}")
        if sol: items.append(f"◎  SOL: ${sol:,.2f}")
        if bnb: items.append(f"🔶 BNB: ${bnb:,.2f}")
        if xrp: items.append(f"✦  XRP: ${xrp:,.4f}")
    except Exception as e: logger.warning(f"Price snap failed: {e}")
    if not items: return f"📰 <b>{BOT_HEADER} Market News</b>\n\nNo news available."
  ...
  def run_backtest(symbol):
    try:
        ...
        results={"WIN":0,"LOSS":0,"SKIP":0}; pattern_res={}
        ...
        logger.error(f"Backtest failed {symbol}: {e}",exc_info=True); return f"Backtest failed: {e}"
...
def get_active_trades_text():
    ...
    for coin,t in active_trades.items():
        text+=f"<b>{coin}</b> {t['direction']}\nEntry: {format_price(t['entry'])} | SL: {format_price(t['sl'])}\nTP: {format_price(t['tp'])} | Lev: {t['leverage']}x\n\n"
    return text
...
def get_10day_summary_text():
    ...
    for days_ago in range(9,-1,-1):
        ...
        total=wins+losses; pnl=sum(t["pnl"] for t in day_trades)
        wr=(wins/total*100) if total>0 else 0
        overall_wins+=wins; overall_losses+=losses; overall_pnl+=pnl
        if total==0: text+=f"<b>{day.strftime('%d %b')}</b> — No trades\n"
        else:
            emoji="✅" if wins>losses else "❌" if losses>wins else "➖"
            text+=f"{emoji} <b>{day.strftime('%d %b')}</b>: {wins}W/{losses}L | WR:{wr:.0f}% | PnL:{pnl:+.1f}%\n"
    ot=overall_wins+overall_losses; owr=(overall_wins/ot*100) if ot>0 else 0
    text+=f"\n<b>━━━ 10 Day Total ━━━</b>\n✅ {overall_wins}W | ❌ {overall_losses}L\n🎯 Win Rate: {owr:.1f}%\n💰 Total PnL: {overall_pnl:+.2f}%\n📊 Avg/day: {overall_pnl/10:+.2f}%"
    return text

def get_streak_text():
    if not trade_journal: return f"🔥 <b>{BOT_HEADER} Streak</b>\n\nNo trades yet."
    ...
    text=f"{emoji} <b>{BOT_HEADER} Streak</b>\n\n{'Winning' if streak_type=='WIN' else 'Losing'}: <b>{streak_count}</b> trades\n"
    if streak_type=="WIN" and streak_count>=3: text+="\n🔥 You're on fire! Stay disciplined."
    elif streak_type=="LOSS" and streak_count>=2: text+="\n⚠️ Losing streak. Consider reducing size."
    return text

def get_best_text():
    if not trade_journal: return f"⭐ <b>{BOT_HEADER} Best</b>\n\nNo data yet."
    coin_stats={}; ps2={}
    for t in trade_journal:
        c=t["coin"]
        if c not in coin_stats: coin_stats[c]={"W":0,"L":0,"pnl":0}
        coin_stats[c]["W" if t["result"]=="WIN" else "L"]+=1; coin_stats[c]["pnl"]+=t["pnl"]
        p=t["pattern"]
        if p not in ps2: ps2[p]={"W":0,"L":0}
        ps2[p]["W" if t["result"]=="WIN" else "L"]+=1
    text=f"⭐ <b>{BOT_HEADER} Best Performers</b>\n\n<b>Top Coins:</b>\n"
    for i,(c,s) in enumerate(sorted(coin_stats.items(),key=lambda x:(x[1]["W"]/(x[1]["W"]+x[1]["L"]) if (x[1]["W"]+x[1]["L"])>0 else 0),reverse=True)[:3],1):
        tot=s["W"]+s["L"]; wr=(s["W"]/tot*100) if tot>0 else 0
        text+=f"{i}. <b>{c}</b> — {wr:.1f}% WR | PnL: {s['pnl']:+.1f}%\n"
    text+="\n<b>Top Patterns:</b>\n"
    for i,(p,s) in enumerate(sorted(ps2.items(),key=lambda x:(x[1]["W"]/(x[1]["W"]+x[1]["L"]) if (x[1]["W"]+x[1]["L"])>0 else 0),reverse=True)[:3],1):
        tot=s["W"]+s["L"]; wr=(s["W"]/tot*100) if tot>0 else 0
...
def get_risk_text():
    ...
    for coin,t in active_trades.items():
        risk_pct=abs(t["entry"]-t["sl"])/t["entry"]*100*t["leverage"]; total_risk+=risk_pct
        text+=f"<b>{coin}</b>: {risk_pct:.1f}% max loss\n"
    text+=f"\n📊 Total Risk: {total_risk:.1f}%\n📌 Active: {len(active_trades)}/{MAX_ACTIVE_TRADES}\n🛡️ CB: {'🔴' if check_circuit_breaker() else '🟢'}"
    return text
...
def check_price_alerts():
    ...
    for symbol,alert in list(price_alerts.items()):
        ...
        if alert["direction"]=="above" and price>=alert["price"]:
            send_telegram(f"🔔 <b>{BOT_HEADER} PRICE ALERT</b>\n\n<b>{symbol}</b> is now ABOVE {format_price(alert['price'])}\nCurrent: {format_price(price)}")
            triggered.append(symbol)
        elif alert["direction"]=="below" and price<=alert["price"]:
            send_telegram(f"🔔 <b>{BOT_HEADER} PRICE ALERT</b>\n\n<b>{symbol}</b> is now BELOW {format_price(alert['price'])}\nCurrent: {format_price(price)}")
            triggered.append(symbol)
    for symbol in triggered: del price_alerts[symbol]
...
def check_partial_tp(coin, trade, price, pnl):
    ...
    try:
        res=requests.get("https://cryptopanic.com/api/v1/posts/",
                         params={"auth_token":NEWS_API_KEY,"currencies":coin,"kind":"news"},timeout=5)
...
def format_and_send(setup, coin, is_river=False, is_instant=False, market_condition="bull"):
    ...
    vol_ok=is_volume_confirmed(klines); rsi_ok=is_rsi_valid(closes,setup["direction"])
    funding_ok=is_funding_favorable(setup["symbol"],setup["direction"])
    if not vol_ok or not rsi_ok or not is_volatility_normal(klines) or not funding_ok: return False
    ...
    mom=(closes[-1]-closes[-3])/closes[-3]*100; rsi_val=calculate_rsi(closes)
    ...
    setup.update({"entry":entry,"sl":sl,"tp":tp,"timestamp":get_ist_datetime(),
                  ...
                  "partial_tp_taken":False,"tf_score":tf_score,"market_condition":market_condition})
...
def send_hourly_batch():
    ...
    for s in sorted_q:
        ...
        if format_and_send(s,s["coin"],market_condition=s.get("market_condition","bull")): sent_count+=1
    for s in sorted_q:
...
def check_active_trades():
    for coin,trade in list(active_trades.items()):
        ...
        update_trailing_sl(coin,trade,price); check_partial_tp(coin,trade,price,pnl)
        ...
        if hit:
            with trade_lock:
                ...
                trade_journal.append({"date":str(datetime.now(IST).date()),"coin":coin,
                    "direction":trade["direction"],"pattern":primary,"entry":trade["entry"],
                    "exit":price,"pnl":pnl,"result":hit,"duration":duration,
            ...
            for update in res.json().get("result",[]):
                ...
                if "callback_query" in update:
                    ...
                    if "ACTIVATE" in data and coin in pending_signals:
                        ...
                        send_telegram(f"❌ <b>{BOT_HEADER}</b> {coin} Ignored"); del pending_signals[coin]
                    ...
                    if   text=="/stats":   send_telegram(get_pattern_stats_text())
                        ...
                        if len(parts)>=4:
                            ...
                            except Exception: send_telegram("❌ Invalid format. Use: /alert BTC 95000 above")
...
def scan_coins(btc_trend, fng, market_condition):
    for coin in COINS:
        try:
            ...
            if tf_score==-1: logger.info(f"Skipping {coin} — 4h disagrees"); continue
            if tf_score<2 and best[1]<96: logger.info(f"Skipping {coin} — TF low"); continue
...
def main():
    ...
    print(STARTUP_ART); logger.info(STARTUP_ART)
    send_telegram(
        f"╔══════════════════════════════════╗\n"
    ...
    logger.info(f"TRADING SIGNAL MASTER {BOT_VERSION} started | {len(COINS)} coins | Score: {MIN_SETUP_SCORE}+")
    while True:
        try:
            ...
            logger.info(f"BTC:{'BULL' if btc_trend==1 else 'BEAR'}|Market:{market_condition}|F&G:{fng}|Losses:{daily_losses}/{MAX_DAILY_LOSSES}")
